...
export default function AdminDashboard() {
  ...
  const { data: currentAdmin, isLoading: adminLoading } = useQuery({
    ...
    enabled: !!adminToken,
    queryFn: async () => {
      const res = await apiRequest('GET', '/api/admin/health', undefined, adminToken);
      return res.json();
    }
  });
  ...
  const { data: metrics, isLoading: metricsLoading } = useQuery({
    ...
    enabled: !!adminToken,
    queryFn: async () => {
      const res = await apiRequest('GET', '/api/admin/dashboard/metrics', undefined, adminToken);
      return res.json();
    }
  });
  ...
  const { data: reviewQueue, isLoading: reviewLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await apiRequest('GET', '/api/admin/moderation/queue', undefined, adminToken);
      return res.json();
    }
  });
  ...
  const { data: flaggedContent, isLoading: flaggedLoading } = useQuery({
    ...
    enabled: !!adminToken && selectedTab === 'moderation',
    queryFn: async () => {
      const res = await apiRequest('GET', '/api/admin/moderation/flagged-content', undefined, adminToken);
      return res.json();
    }
  });
  ...
  const logoutMutation = useMutation({
    mutationFn: async () => {
      const res = await apiRequest('POST', '/api/admin/auth/logout', undefined, adminToken);
      await res.json();
    },
    onSuccess: () => {
      setAdminToken(null);
  ...
  const processReviewMutation = useMutation({
    mutationFn: async ({ itemId, action, reason }: { itemId: number; action: string; reason: string }) => {
      const res = await apiRequest('POST', `/api/admin/moderation/queue/${itemId}/process`, { action, reason }, adminToken);
      return res.json();
    },
    onSuccess: () => {
  ...
  const searchUsersMutation = useMutation({
    mutationFn: async (query: string) => {
      const res = await apiRequest('GET', `/api/admin/users/search?q=${encodeURIComponent(query)}`, undefined, adminToken);
      return res.json();
    }
  });
  ...
  const banUserMutation = useMutation({
    mutationFn: async ({ userId, reason, duration }: { userId: number; reason: string; duration?: number }) => {
      const res = await apiRequest('POST', `/api/admin/users/${userId}/ban`, { reason, duration }, adminToken);
      return res.json();
    },
    onSuccess: () => {
      toast({
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="flex justify-between items-center mb-8">
        ...
        <div className="flex items-center gap-4">
          <Badge variant={currentAdmin?.systemHealth === 'excellent' ? 'default' : 'destructive'}>
            System: {currentAdmin?.systemHealth || 'Unknown'}
          </Badge>
          
          <div className="text-right">
            <p className="font-medium">{currentAdmin?.activeAdmin?.username}</p>
            <p className="text-sm text-gray-500">{currentAdmin?.activeAdmin?.role}</p>
          </div>
          ...
          {metricsLoading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              ...
              <MetricCard
                ...
                variant={metrics?.pendingReviews > 0 ? "warning" : "default"}
              />
            </div>
          )}
              ...
              <CardContent>
                {reviewLoading ? (
                  ...
                  <div className="space-y-4">
                    {reviewQueue?.map((item: ReviewItem) => (
              ...
              <CardContent>
                {flaggedLoading ? (
                  ...
                  <div className="space-y-3">
                    {flaggedContent?.slice(0, 5).map((flag: any) => (
