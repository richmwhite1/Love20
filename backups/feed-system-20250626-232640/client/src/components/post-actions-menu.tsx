...
export function PostActionsMenu({ postId, post, onEdit, onDelete }: PostActionsMenuProps) {
  ...
  const tagFriendsMutation = useMutation({
    mutationFn: async (userIds: number[]) => {
      ...
      const response = await apiRequest('POST', `/api/posts/${postId}/tag`, { userIds }, token);
  ...
  const repostMutation = useMutation({
    mutationFn: async () => {
      const token = await getIdToken();
      const response = await apiRequest('POST', `/api/posts/${postId}/repost`, undefined, token);
      return response.json();
    },
    onSuccess: () => {
  ...
  const savePostMutation = useMutation({
    mutationFn: async (listId: number) => {
      const token = await getIdToken();
      const response = await apiRequest('POST', `/api/posts/${postId}/save`, { listId }, token);
      return response.json();
    },
    onSuccess: () => {
      toast({ 
  ...
  const flagPostMutation = useMutation({
    mutationFn: async (reason: string) => {
      const token = await getIdToken();
      const response = await apiRequest('POST', `/api/posts/${postId}/flag`, { reason }, token);
      return response.json();
    },
  ...
  const deletePostMutation = useMutation({
    mutationFn: async () => {
      const token = await getIdToken();
      const response = await apiRequest('DELETE', `/api/posts/${postId}`, undefined, token);
      return response.json();
    },
    onSuccess: () => {
  ...
  const handleShare = () => {
    if (navigator.share) {
      navigator.share({
        title: post.primaryDescription || 'Check out this post',
  ...
  const submitTags = () => {
    if (selectedFriends.length > 0) {
      tagFriendsMutation.mutate(selectedFriends.map(id => parseInt(id)));
    }
  };

  const submitSave = () => {
    if (selectedList) {
      savePostMutation.mutate(parseInt(selectedList));
    }
  };
