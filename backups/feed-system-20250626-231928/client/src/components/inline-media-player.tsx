...
class AudioManager {
  ...
  subscribe(callback: (postId: string | null) => void) {
    this.listeners.add(callback);
    return () => this.listeners.delete(callback);
  }

  play(audio: HTMLAudioElement | null, postId: string) {
...
export default function InlineMediaPlayer({ youtubeUrl, spotifyUrl, postId, thumbnailUrl, onPostClick }: InlineMediaPlayerProps) {
  ...
  useEffect(() => {
    const unsubscribe = audioManager.subscribe((currentPostId) => {
      setIsPlaying(currentPostId === postId);
    });
    return () => {
      unsubscribe();
    };
  ...
  return (
    <div className="relative w-full h-96">
      ...
      <audio
        ref={audioRef}
        src={(youtubeUrl || spotifyUrl) || undefined}
        onEnded={handleEnded}
        preload="metadata"
      />
    </div>
