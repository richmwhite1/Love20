...
import { Search as SearchIcon, Hash, TrendingUp, X, SortAsc, ChevronDown, ChevronUp, Users, UserPlus, UserCheck, UserX, Check, Plus } from "lucide-react";
...
export default function SearchPage() {
  ...
  const { data: searchResults, isLoading: searchLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await api.search.hashtags(`${hashtagsParam}&sort=${sortBy}`);
      if (!response.success) throw new Error(response.error || 'Failed to search posts');
  ...
  const { data: trendingHashtags } = useQuery({
    ...
    queryFn: async () => {
      const response = await api.hashtags.getTrending();
      if (!response.success) throw new Error(response.error || 'Failed to fetch trending hashtags');
      return response.data || [];
    },
  ...
  const { data: followedHashtags, refetch: refetchFollowed } = useQuery({
    ...
    queryFn: async () => {
      const response = await api.hashtags.getFollowed();
      if (!response.success) throw new Error(response.error || 'Failed to fetch followed hashtags');
      return response.data || [];
    },
  ...
  const followHashtag = async (hashtagId: number) => {
    try {
      const response = await api.hashtags.follow(hashtagId.toString());
      if (response.success) {
        refetchFollowed();
      }
    } catch (error) {
  ...
  const unfollowHashtag = async (hashtagId: number) => {
    try {
      const response = await api.hashtags.unfollow(hashtagId.toString());
      if (response.success) {
        refetchFollowed();
      }
    } catch (error) {
  ...
  const useIsFollowingHashtag = (hashtagId: number) => {
    return useQuery({
      ...
      queryFn: async () => {
        const response = await api.hashtags.following(hashtagId.toString());
        if (!response.success) return false;
        return (response.data as any)?.isFollowing || false;
      },
  ...
  const handleFollow = async (userId: string) => {
    try {
      const response = await api.connections.sendRequest(userId);
      if (response.success) {
        toast({
  ...
  const handleUnfollow = async (userId: string) => {
    try {
      const response = await api.connections.remove(userId);
      if (response.success) {
        toast({
  ...
  const handleAcceptRequest = async (requestId: string) => {
    try {
      const response = await api.connections.acceptRequest(requestId);
      if (response.success) {
        toast({
  ...
  const handleRejectRequest = async (requestId: string) => {
    try {
      const response = await api.connections.rejectRequest(requestId);
      if (response.success) {
        toast({
  ...
  const safeTrendingHashtags = Array.isArray(trendingHashtags) ? trendingHashtags : [];
  const safeFollowedHashtags = Array.isArray(followedHashtags) ? followedHashtags : [];
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
              ...
              {isFollowingExpanded && (
                <CardContent className="pt-0">
                  <div className="flex flex-wrap gap-2">
                    {safeFollowedHashtags.map((hashtag: any) => (
          ...
          <Card>
            ...
            {isTrendingExpanded && (
              <CardContent className="pt-0">
                <div className="flex flex-wrap gap-2">
                  {safeTrendingHashtags.map((hashtag: any) => {
                  const FollowButton = () => {
          ...
          <div>
            ...
            {searchLoading ? (
              ...
              <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                {safeSearchResults.map((post: any) => (
