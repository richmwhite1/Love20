...
export default function EventRsvp({ post }: EventRsvpProps) {
  ...
  const { data: userRsvp } = useQuery<{ status: string }>({
    queryKey: [`/api/posts/${post.id}/rsvp`],
    enabled: !!user,
  });

  // Get RSVP statistics
  const { data: rsvpStats } = useQuery<{ going: number; maybe: number; not_going: number }>({
  ...
  const rsvpMutation = useMutation({
    mutationFn: async (status: string) => {
      const response = await api.events.rsvp(post.id, status);
      if (!response.success) {
